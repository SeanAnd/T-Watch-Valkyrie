...
- **Classifier** (ported from `WiFiScan.cpp` lines 894-989 of upstream ESP32Valkyrie where applicable): AirTag / Find My (TLV-aware manufacturer `0x004C` parsing plus legacy sliding-window fallbacks, including `4C 00 07 19` style payloads), Flipper Zero, HC-03/05/06 skimmer, Flock camera, Meta Ray-Ban / Quest smart glasses, drone RemoteID broadcasts over BLE (service `0xFFFA`). **Wi‑Fi drone fingerprint** (no OpenDroneID decode): during the promiscuous pass, management frames matching the Sky-Spy NAN destination MAC `51:6f:9a:01:00:00` or beacons with vendor IE `0xDD` + OUI `90:3a:e6` / `fa:0b:bc` emit `ThreatType::Drone` when Drone is enabled in Threats — same frame shapes [Sky-Spy](https://github.com/colonelpanichacks/Sky-Spy) uses before full parse; we stop at detection-only to save flash.
...
- **Power-aware scheduling:** duty-cycled BLE scan window (defaults ~10 s scan every ~30 s between **threat pass** starts; idle gap is `max(0, scanIntervalSecs - scanWindowSecs)` in NVS). Each pass ends with a short Wi‑Fi promiscuous hop on channels 1 / 6 / 11 (Flock Wi‑Fi heuristics, optional Wi‑Fi threat toggles, and drone Wi‑Fi fingerprint when Drone is on), then hub timing updates. Gated on `PowerFSM` state and battery percentage, aborted on `notifyDeepSleep` / `notifyLightSleep`.
...
**Wi‑Fi RemoteID:** on-watch detection uses **fingerprints only** (see Features). Full OpenDroneID message decode (Basic ID, GPS, operator location) and USB serial JSON for tools like [drone-mesh-mapper](https://github.com/colonelpanichacks/drone-mesh-mapper) are **not** implemented here; add later if you need mapper-style telemetry.
...
- **[colonel panic hacks](https://github.com/colonelpanichacks):** `flock-you` infrastructure OUI list for Flock, **Sky-Spy**-style ASTM F3411 RemoteID over BLE (service UUID 0xFFFA), and the same Wi‑Fi NAN / beacon vendor-IE fingerprints Sky-Spy uses for Wi‑Fi RemoteID (detection-only here, no OpenDroneID decode).
